...
USER node
WORKDIR /srv/app/apps/worker
# Dependency readiness only; job completion must also be monitored in BullMQ.
HEALTHCHECK --interval=30s --timeout=10s --start-period=30s --retries=3 \
  CMD node /srv/app/infra/docker/worker-healthcheck.mjs
