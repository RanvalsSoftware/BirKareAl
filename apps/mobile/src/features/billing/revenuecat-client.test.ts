...
function info(active = false, marker = 'now'): CustomerInfo {
  return { requestDate: marker, entitlements: { active: active ? {
    [entitlementId]: { isActive: true, productIdentifier: 'monthly', willRenew: true },
  } : {} } } as unknown as CustomerInfo;
}
const pkg = { identifier: '$rc_monthly', product: { identifier: 'monthly', priceString: '₺99,99' } } as PurchasesPackage;
const offering = { identifier: 'default', availablePackages: [pkg], monthly: pkg } as PurchasesOffering;
function deferred<T>() {
...
function fixture(unavailableReason?: string) {
  ...
  const sdk = {
    ...
    addCustomerInfoUpdateListener: vi.fn((callback: (value: CustomerInfo) => void) => { listener = callback; }),
  ...
  const loadSdk = vi.fn(async () => ({ default: sdk, PURCHASES_ERROR_CODE: {
    PURCHASE_CANCELLED_ERROR: '1', PAYMENT_PENDING_ERROR: '20', NETWORK_ERROR: '10', CONFIGURATION_ERROR: '23',
  } }) as unknown as typeof import('react-native-purchases'));
  const client = createRevenueCatClient({ apiKey: 'test_fixture', entitlementId, unavailableReason,
    getUserId: () => user, loadSdk,
    loadUi: async () => ({ default: ui, PAYWALL_RESULT: {
      PURCHASED: 'PURCHASED', RESTORED: 'RESTORED', CANCELLED: 'CANCELLED', ERROR: 'ERROR', NOT_PRESENTED: 'NOT_PRESENTED',
    } }) as unknown as typeof import('react-native-purchases-ui'),
  });
  return { client, sdk, ui, loadSdk,
    signIn: async () => client.setUser(user),
    setUser(value: string | null) { user = value; return client.setUser(value); },
    setInfo(value: CustomerInfo) { latestInfo = value; },
    emit(value: CustomerInfo) { listener?.(value); },
  };
}
...
describe('RevenueCat account and purchase lifecycle', () => {
  ...
  it('configures once using the authenticated backend user ID', async () => {
    const f = fixture(); await f.signIn(); await f.signIn();
    expect(f.sdk.configure).toHaveBeenCalledExactlyOnceWith({ apiKey: 'test_fixture', appUserID: 'user-a' });
    expect(f.client.getSnapshot().status).toBe('ready');
  });
  it('does not load SDKs on unsupported platforms or Expo Go', async () => {
    const f = fixture('Native build required'); await f.signIn();
  ...
  it('clears entitlement synchronously on account switch', async () => {
    const f = fixture(); f.setInfo(info(true)); await f.signIn();
    f.setInfo(info()); const login = f.setUser('user-b');
  ...
  it('logs out and blocks purchases without an app account', async () => {
    ...
    expect(f.client.getSnapshot().customerInfo).toBeNull(); await logout;
  ...
  it('discards a late customer response after switching accounts', async () => {
    const f = fixture(); const read = deferred<CustomerInfo>();
    f.sdk.getCustomerInfo.mockImplementationOnce(() => read.promise);
    const first = f.signIn(); await vi.waitFor(() => expect(f.sdk.getCustomerInfo).toHaveBeenCalled());
    const second = f.setUser('user-b'); read.resolve(info(true, 'old-account'));
    await Promise.all([first, second]);
    expect(f.client.getSnapshot().userId).toBe('user-b');
    expect(hasPro(f.client.getSnapshot().customerInfo, entitlementId)).toBe(false);
  });
  it('discards a completed purchase from a previous app account', async () => {
    const f = fixture(); await f.signIn(); const buy = deferred<{ customerInfo: CustomerInfo }>();
    ...
    const login = f.setUser('user-b'); buy.resolve({ customerInfo: info(true) });
    expect((await purchase).kind).toBe('cancelled'); await login;
    expect(hasPro(f.client.getSnapshot().customerInfo, entitlementId)).toBe(false);
  });
  it('purchases the exact package from the offering', async () => {
    const f = fixture(); await f.signIn(); f.setInfo(info(true));
  ...
  it('does not start duplicate purchases on repeated taps', async () => {
    ...
    expect((await f.client.purchase(pkg)).kind).toBe('error');
    buy.resolve({ customerInfo: info(true) }); await first;
  ...
  it('treats user cancellation as cancellation, without an error banner', async () => {
    const f = fixture(); await f.signIn(); f.sdk.purchasePackage.mockRejectedValueOnce({ userCancelled: true, code: '1' });
  ...
  it('never grants Pro for a pending payment', async () => {
    const f = fixture(); await f.signIn(); f.sdk.purchasePackage.mockRejectedValueOnce({ code: '20' });
    expect((await f.client.purchase(pkg)).kind).toBe('pending');
    expect(hasPro(f.client.getSnapshot().customerInfo, entitlementId)).toBe(false);
  });
  it('handles network errors and permits an explicit retry', async () => {
    const f = fixture(); await f.signIn(); f.sdk.purchasePackage.mockRejectedValueOnce({ code: '10' });
    expect((await f.client.purchase(pkg)).kind).toBe('error');
    f.setInfo(info(true)); expect((await f.client.purchase(pkg)).kind).toBe('completed');
  });
  it('restores only the entitlement actually returned by RevenueCat', async () => {
    ...
    f.setInfo(info(true)); expect(await f.client.restore()).toEqual({ kind: 'completed', isPro: true });
  });
  it('keeps restore available when fetching offerings fails', async () => {
    const f = fixture(); f.sdk.getOfferings.mockRejectedValueOnce({ code: '10' }); await f.signIn();
    expect(f.client.getSnapshot().status).toBe('ready');
    f.setInfo(info(true)); expect(await f.client.restore()).toEqual({ kind: 'completed', isPro: true });
  });
  it('shows the native paywall for the exact entitlement and offering', async () => {
    ...
    expect(f.ui.presentPaywallIfNeeded).toHaveBeenCalledWith({
      requiredEntitlementIdentifier: entitlementId, offering, displayCloseButton: true,
    });
  });
  ...
  it('does not grant another account’s entitlement from a delayed listener payload', async () => {
    const f = fixture(); await f.signIn();
    f.emit(info(true, 'delayed-other-account')); await f.client.refresh();
    expect(hasPro(f.client.getSnapshot().customerInfo, entitlementId)).toBe(false);
  });
  it('refresh revokes expired entitlements instead of retaining a local Pro boolean', async () => {
    const f = fixture(); f.setInfo(info(true)); await f.signIn();
    f.setInfo(info(false, 'expired')); await f.client.refresh();
    expect(hasPro(f.client.getSnapshot().customerInfo, entitlementId)).toBe(false);
  });
  it('cleans up its single native customer info listener', async () => {
    const f = fixture(); await f.signIn(); f.client.dispose();
