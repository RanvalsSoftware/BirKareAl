...
OPENAI_IMAGE_PREMIUM_MODEL: ${OPENAI_IMAGE_PREMIUM_MODEL:-gpt-image-2.5-sunburst}
...
volumes: - birkare-storage:/srv/app/.local-storage
depends_on:
cloudsql-proxy:
...
platform: linux/amd64
restart: unless-stopped
command: - --address=0.0.0.0 - --port=5432 - --structured-logs - --health-check - --credentials-file=/run/secrets/cloudsql-service-account - ${CLOUD_SQL_INSTANCE_CONNECTION_NAME:?Set Cloud SQL instance connection name}
