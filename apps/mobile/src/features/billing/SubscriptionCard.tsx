...
function notify(result: BillingResult, restoring = false) {
  ...
  Alert.alert('BirKare Pro', result.isPro
...
export function SubscriptionCard() {
  ...
  ].filter((plan): plan is { label: string; note: string; pkg: PurchasesPackage } => Boolean(plan.pkg));
  const disabled = !billing.ready || billing.busy;
  const expiration = billing.entitlement?.expirationDate;
  const expiresText = expiration && Number.isFinite(Date.parse(expiration))
    ? new Date(expiration).toLocaleDateString('tr-TR') : null;

  return (
    <View style={styles.card}>
      <ProBadge label={billing.isPro ? 'BirKare Pro · Aktif' : 'BirKare Pro'} />
      <Text style={styles.title}>{billing.isPro ? 'Pro hesabınız' : 'Size uygun Pro planını seçin.'}</Text>
      {billing.isTestStore ? <Text style={styles.test}>TEST ORTAMI · Gerçek ücret alınmaz.</Text> : null}
      <Text style={styles.text}>
        Pro erişimi ve üretim kredileri ayrı yönetilir. Bu satın alma bağlantısı kredi bakiyesine otomatik yükleme yapmaz.
      </Text>
      {billing.isPro && expiresText ? <Text style={styles.text}>
        {billing.entitlement?.willRenew ? 'Yenileme tarihi' : 'Erişim bitişi'}: {expiresText}
      </Text> : null}
      {billing.isPro && !expiration ? <Text style={styles.text}>Süresiz Pro erişimi · Otomatik yenileme yok.</Text> : null}
      {billing.status === 'connecting' ? <Text style={styles.text}>Mağaza hazırlanıyor…</Text> : null}
      {billing.status === 'signed_out' ? <Text style={styles.text}>Paketleri görmek için hesabınıza giriş yapın.</Text> : null}
      {billing.error ? <Text accessibilityRole="alert" style={styles.warning}>{billing.error}</Text> : null}

      {!billing.isPro ? plans.map(({ label, note, pkg }) => (
        <Pressable key={pkg.identifier} accessibilityRole="button"
          accessibilityLabel={`${label}, ${pkg.product.priceString}, ${note}`}
          disabled={disabled} style={[styles.plan, disabled && styles.disabled]}
          onPress={() => { void billing.purchase(pkg).then((result) => notify(result)); }}>
      ...
      {!billing.isPro && billing.ready && offering && plans.length === 0 ?
        <Text style={styles.warning}>Current offering içinde Monthly, Annual veya Lifetime paket türü bulunamadı.</Text> : null}
      {!billing.isPro ? <Pressable accessibilityRole="button"
        disabled={disabled || !offering} style={[styles.primary, (disabled || !offering) && styles.disabled]}
        onPress={() => { void billing.presentPaywall().then((result) => notify(result)); }}>
        <Text style={styles.primaryText}>{billing.busy ? 'İşlem sürüyor…' : 'Pro teklifini aç'}</Text>
      </Pressable> : null}
      <View style={styles.links}>
        <Pressable accessibilityRole="button" disabled={disabled} style={styles.linkButton}
        ...
        <Pressable accessibilityRole="button" disabled={disabled} style={styles.linkButton}
          onPress={() => { void billing.presentCustomerCenter().then((result) => {
        ...
        <Pressable accessibilityRole="button" disabled={billing.busy} style={styles.linkButton}
          onPress={() => { void refresh(); }}>
          <Text style={styles.link}>Durumu yenile</Text>
        </Pressable>
      </View>
      <Text style={styles.finePrint}>
        Aylık ve yıllık planlar mağazadan iptal edilmedikçe yenilenir. Ömür boyu plan tek seferliktir.
        Toplam fiyat ve satın alma koşulları mağazanın onay ekranında gösterilir.
      </Text>
      <Pressable accessibilityRole="link" style={styles.linkButton} onPress={() => router.push('/legal')}>
...
const styles = StyleSheet.create({
  card: { marginTop: spacing.md, padding: spacing.lg, backgroundColor: colors.surfaceElevated,
    borderRadius: radii.lg, borderWidth: 1, borderColor: 'rgba(255,196,0,0.31)' },
  ...
  plan: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12,
    backgroundColor: colors.surface, padding: 14, borderRadius: 12, marginTop: 10,
    borderColor: colors.border, borderWidth: 1, minHeight: 68 },
  ...
  primary: { minHeight: 48, borderRadius: 12, backgroundColor: colors.accentYellow,
    alignItems: 'center', justifyContent: 'center', marginTop: 14, paddingHorizontal: 12 },
