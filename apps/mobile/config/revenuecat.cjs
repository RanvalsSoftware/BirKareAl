...
function resolveRevenueCatConfig(values) {
  ...
  const strictStoreRelease =
    appEnv === 'production' ||
    values.EAS_BUILD_PROFILE === 'production' ||
  ...
  function validate(value, name, prefix) {
    ...
    if (value.startsWith('test_')) {
      if (strictStoreRelease)
        throw new Error('RevenueCat Test Store keys cannot be used in production builds.');
      return value;
    }
  ...
  const androidApiKey = validate(
    values.EXPO_PUBLIC_REVENUECAT_ANDROID_API_KEY?.trim() ||
      (strictStoreRelease || (release && buildPlatform !== 'android')
        ? ''
